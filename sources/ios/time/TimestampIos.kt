...
actual val Timestamp.dayOfWeek
	get() = when (val weekday = Clock.gregorian.platform.component(NSWeekdayCalendarUnit, fromDate = toPlatform())) {
		1L -> DayOfWeek.sunday
		2L -> DayOfWeek.monday
...
actual fun Timestamp.toLocalDate(timeZone: TimeZone): LocalDate {
	val components = Clock.gregorian.platform.componentsInTimeZone(timezone = timeZone.platform, fromDate = toPlatform())
	return LocalDate.of(
...
fun PlatformTimestamp.toCommon(): Timestamp {
	...
	return Timestamp.of(
		...
		nanoseconds = Nanoseconds(((secondsSince1970 % 1) * 1_000_000_000).roundToLong())
	)
}
